...
@Suite("H.264 missing composition timestamp recovery")
struct H264CompositionTimestampProbeTests {
    ...
    @Test("two raw/best differences confirm repair even without a measured backward step")
    func bestEffortDifferenceConfirmsRepair() {
        var evidence = completeEvidence()
        evidence.rawPTSRegressions = 0
        evidence.rawPTSDiffersFromBestEffort = 2
        #expect(H264CompositionTimestampProbe.classify(evidence)
            == .repairWithBestEffortPTS)
    ...
    @Test("decoded frame timestamp policy is opt-in and falls back on NOPTS")
    ...
    /// 96x64 H.264 Main, 30000/1001 fps, 66 frames, three B-frames per group. The healthy fixture
    /// contains a `ctts` table; the malformed twin was stream-copied through
    /// `-bsf:v setts=pts=DTS`, preserving the reordered bitstream while deleting composition
    /// offsets. Both stay tiny because every frame is the same color.
