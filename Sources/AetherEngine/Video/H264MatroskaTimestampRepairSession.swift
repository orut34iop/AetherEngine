...
final class H264MatroskaTimestampRepairSession: H264TimestampRepairSession {
    ...
    private func finishSequence(nextPTS: Int64) throws {
        ...
        if lead == nil {
            ...
            verdict = "confirmedMatroskaCodingOrder"
    ...
    private func refuse(healthy: Bool) throws {
        if lead != nil {
            ...
            failed = true
            verdict = "matroskaSequenceChanged"
            releasePackets()
        ...
        verdict = healthy ? "healthy" : "matroskaSequenceUnproven"
        off = true
        publishPending()
    }
    ...
    func noteSeek() {
        ...
        reader.reset()
        failed = false
        if lead != nil { verdict = "confirmedMatroskaCodingOrder" }
    ...
    var summary: String {
        "verdict=\(verdict) samples=\(sampledCount) held_packets=\(sampledPackets)"
            + " held_bytes=\(sampledBytes) poc_regressions=\(sampledRegressions)"
            + " decode_lead=\(lead ?? 0) repaired=\(repairedCount) unrepaired=\(unrepairedCount)"
    }
}
