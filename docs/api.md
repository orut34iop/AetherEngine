...
## Moonfin fork additions (6.71.0 integration)

These additions preserve the downstream tvOS contract; they are not claims about the upstream release.

- `EngineTLS.Origin` and `allowedUntrustedCertificateOrigins` retain exact HTTPS
  scheme/host/effective-port approval. `allowsUntrustedCertificate(for:)` queries that
  set. The upstream `serverTrustEvaluator` remains independent; a host should choose
  one policy surface. Moonfin leaves the evaluator nil and never installs an accept-all
  closure. The upstream HLS relay applies the same policy at each actual handshake.
- `activeSecondarySubtitleTrackIndex`, `secondaryBitmapSupported` (false), and
  `secondarySidecarASSHeader` preserve dual text/ASS subtitle identity and styling.
  One track cannot occupy both channels. Inherited sidecar headers remain exact-origin,
  including the new remote-HLS injected rendition path.
- `LoadOptions.sessionCacheByteBudget` and `sessionCacheStatus` preserve the requested
  byte budget, playback-safe floor and cleanup telemetry. Types: `SessionCacheStatus`,
  `SessionCacheRoute`, `SessionCacheCapability`, `SessionCacheCleanupReason`,
  `SessionCacheCleanupResult`, and `SessionCacheFailureReason`.
  Bounded stale cleanup composes with upstream live-session locks and directory recovery.
- `DisplayModeDiagnostic` and `H264CompositionOffsetRepairDiagnostic` are identity-free
  measurements exposed by `EngineDiagnostics` (`displayModeDiagnostic` and
  `h264CompositionOffsetRepairDiagnostic`). `H264CompositionOffsetRepairOutcome` and
  `H264CompositionOffsetRepairReason` distinguish eligibility, verdict and counters.
  The repair algorithm itself is upstream's fractional-cadence implementation.
...
| `setNativeVideoFrameTimeObserver(_:)` | Takes a `NativeVideoFrameTimeObserver`, called per muxed frame on both axes (`NativeVideoFrameTime`: `source`, `item`, `segmentIndex`, `isKeyframe`, `epoch`). Called on the producer pump thread in **decode** order, so `source` is not monotonic under B-frames. `epoch` rises process-wide. Optional `sourceDecode`, `itemDecode`, `sampleDuration`, `h264NALTypeMask` and `muxWriteResult` expose repaired source DTS, sanitized mux DTS, mux sample duration, observed H.264 NAL types and libavformat's write result for post-seek diagnostics. They describe mux input, not displayed frames; bit 5 of the NAL mask identifies an observed IDR, unlike the container's `isKeyframe` flag. Existing initializers remain source compatible with nil defaults. |
...
| `LiveTelemetry.softwareCacheSeekHits`, `softwareCacheSeekMisses`, `softwareCacheSourceEpoch` | Optional, session-local software VOD packet-cache counters; nil on other paths. Hits replay retained compressed packets without repositioning the source or discarding its in-flight read. Misses require a source seek and change the source epoch. Existing initializers default these fields to nil. `cachedBytes` includes retained packet history; `bufferedPosition` remains the continuous selected A/V frontier, not a byte-to-time estimate. History is bounded and session-only: an evicted keyframe or a target outside verified coverage can miss. H.264 coverage uses presentation successors under the renderer's held-frame policy; it never rewrites source PTS/DTS/duration. |
