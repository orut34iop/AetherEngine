...
Seeks are demand-driven: `AVPlayer` just fetches segments at the new position, and `VideoSegmentProvider` only tears the producer down and re-anchors it at the requested index (`restartHandler`, burst-coalesced by `RestartCoalescer`) when the request cannot be served from `SegmentCache`. A restart is the expensive path (it re-seeks the demuxer, slow on remote sources, #93), so the cache retains already-produced segments beyond its hard `[target - backwardWindow, target + forwardWindow]` window under a byte budget (automatic default 2 GiB, clamped to a quarter of the tmp volume's free capacity), evicting farthest-from-target first once it fills. A host may instead provide the exact current-session request through `LoadOptions.sessionCacheByteBudget`; zero keeps only the hard playback window. The hard window itself is never evicted, so `SessionCacheStatus.effectiveBudgetBytes` rises to its actual bytes and marks `hardWindowFloorExceeded` when it must exceed the clamped request rather than starving AVPlayer. A larger window is an explicit host opt-in into a whole-source prefetch (`LoadOptions.forwardBufferSegments`, up to 2700 segments ~ 3 h, #207): the producer parks once its race-ahead owns the effective base budget and still keeps ten segments of safety, resuming as eviction behind the playhead frees room (`PrefetchDiskBudget`). A seek back into the retained span, and the forward march that follows it, is then a pure cache hit with zero producer restarts; only a seek into never-produced content restarts. Live/DVR uses the same session budget so advertised rewind history is not deleted prematurely. Native remote HLS bypasses this pipeline and reports `.unsupported`, because AVPlayer and the origin own buffering there. Every loopback session gets a fresh UUID directory; stop, load failure and source replacement close it, while startup performs a bounded one-hour crash-remnant sweep that preserves the current and fresh sibling sessions. No segment is reused across sessions. When a non-disc VOD restart is required, it seeks on the video stream's native timestamp axis to the exact IRAP stored in the segment plan, with that timestamp as the lower bound; a global time seek can land a whole GOP earlier on multi-stream MP4 and turn the producer's scan-forward gate into a long remote read (#191). Disc plans use folded multi-clip timestamps and keep the global-time seek. Restarts on a slow link are further contained (#93 residual): a fetch that is waiting for an in-flight restart rides its progress instead of burning a fixed retry budget into a 503 (and never re-fires a restart at its own stale index), the wedged-restart fresh reopen skips `find_stream_info` (the session already holds saved codec configs and the segment plan), lazy native subtitle readers defer while a restart executes, and the FIRST producer of a resumed session anchors directly at the resume segment instead of producing seg0 into an immediate teardown. Retained scrub bands leave interior holes inside the cache's stored min/max index range, and residency there is not proof a segment exists: a fetch inside the range waits (2 s) only when the active producer's forward march actually covers the requested index, and restarts immediately otherwise (#129).
