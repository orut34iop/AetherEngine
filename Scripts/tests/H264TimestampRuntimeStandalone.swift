...
struct H264TimestampRuntimeTests {
    ...
    static func main() throws {
        ...
        for position in positions.isEmpty ? [0] : positions {
            ...
            if expectedRepair {
                precondition(rawRegressions > 0 && diagnostic.reason == (matroska ? .confirmedMatroskaCodingOrder : .confirmedMissingOffsets))
            ...
            } else { precondition(rawRegressions == 0 && diagnostic.outcome == .healthy) }
